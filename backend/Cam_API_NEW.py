#!/usr/bin/python
#Camera_API_May23.py
#dev by EinsbernSystems (JohnPaulLegaspi)
...
def beep(times=1, duration=0.2, continuous=False):
...
def pir_monitor_thread():
    ...
    if not HAS_GPIO:
        return
    debounce_time = 1.0  # seconds PIR must stay HIGH to count as motion
    min_interval = 5.0   # seconds between notifications even if PIR keeps triggering
    last_motion_time = 0
    while True:
        ...
        if pir_state and motion_sensor_enabled:
            if not pir_last_state:  # Rising edge: motion just detected
                ...
                pir_notified = False
            # Check if PIR has stayed HIGH for debounce_time
            if not pir_notified and (now - pir_triggered_time) >= debounce_time:
                # Only notify if cooldown and min_interval have passed
                if (now - last_notification_time.get('pir_motion', 0) > NOTIFICATION_COOLDOWN and
                    now - last_motion_time > min_interval):
                    ...
                    # Log motion event only when notification is sent
                    log_detection_event('motion', now - (recording_start_time or now), {})
                    flicker_led()
                    threading.Thread(target=beep, args=(2, 0.15)).start()
                    pir_notified = True
        else:
            pir_triggered_time = 0
            pir_notified = False
...
def set_servo_angle():
    ...
    # Allow negative angles, map -90 to 0, 0 to 90, 90 to 180
    if angle is None or not (-90 <= angle <= 180):
        return jsonify({'error': 'Invalid angle'}), 400
    # Map -90 to 0, 0 to 90, 90 to 180
    mapped_angle = angle + 90
    duty_cycle = 2.5 + (mapped_angle / 180.0) * 10
    servo.ChangeDutyCycle(duty_cycle)
    time.sleep(0.3)
    servo.ChangeDutyCycle(0)
    return jsonify({'status': 'success', 'angle': angle, 'mapped_angle': mapped_angle}), 200
...
def snap_picture():
    ...
    # Log the snap event if recording
    if recording and recording_start_time is not None:
        rel_time = time.time() - recording_start_time
        log_detection_event('snap', rel_time, {})
...
def log_detection_event(event_type, timestamp, extra=None):
    if recording and recording_folder and recording_filename:
        ...
        # Log both relative and absolute time
        with open(log_path, 'a') as f:
            f.write(f"{event_type}|{timestamp}|{abs_time}|{extra_str}\n")
...
def recognize_and_draw(frame):
    ...
    if not hasattr(recognize_and_draw, "last_known_time"):
        recognize_and_draw.last_known_time = 0
    if not hasattr(recognize_and_draw, "consecutive_unknowns"):
        recognize_and_draw.consecutive_unknowns = 0
    ...
    with beep_lock:
        if "Unknown" in names:
            # If a known face was seen recently, ignore this unknown
            if now - recognize_and_draw.last_known_time < 2.0:
                recognize_and_draw.consecutive_unknowns = 0
                currentname = "Unknown"
                intruder_last_seen = now
                return frame
            else:
                recognize_and_draw.consecutive_unknowns += 1
            # Only trigger if enough consecutive unknowns
            if recognize_and_draw.consecutive_unknowns >= 3:
                # Flicker LED when unknown detected
                if led_enabled:
                    threading.Thread(target=flicker_led, args=(5,), daemon=True).start()  # Flicker for 5 seconds
                now_time = time.time()
                ...
                # --- Long beep for 5 seconds when unknown detected ---
                if now - last_beep_time > 5.0:
                    threading.Thread(target=beep, kwargs={'duration': 5, 'continuous': True}, daemon=True).start()
                    last_beep_time = now
                currentname = "Unknown"
                intruder_last_seen = now
            ...
            recognize_and_draw.last_known_time = now
            recognize_and_draw.consecutive_unknowns = 0
            ...
            for name in names:
                if name != "Unknown":
                    ...
                    if (name not in last_notification_time or
                        ...
                        # --- Send photo to Telegram for known person ---
                        snap_filename = f"known_{name}_{int(current_time)}.jpg"
                        snap_filepath = os.path.join('/tmp', snap_filename)
                        cv2.imwrite(snap_filepath, frame)

                        # --- Timeline: get last 5 log events if available ---
                        timeline = ""
                        if recording_folder and recording_filename:
                            log_path = get_log_txt_path(recording_folder, recording_filename)
                            if os.path.exists(log_path):
                                try:
                                    with open(log_path, 'r') as f:
                                        lines = f.readlines()
                                    # Only include events up to the current photo event (relative time <= now - recording_start_time)
                                    now_rel = now - (recording_start_time or now)
                                    filtered_lines = []
                                    for line in lines:
                                        parts = line.strip().split('|', 3)
                                        if len(parts) >= 4:
                                            event_type, rel_time, abs_time, extra_str = parts
                                            try:
                                                rel_time_f = float(rel_time)
                                            except Exception:
                                                continue
                                            if rel_time_f > now_rel:
                                                break
                                            if event_type in ('known', 'unknown', 'snap'):
                                                try:
                                                    extra = json.loads(extra_str)
                                                    name = extra.get('name', 'Someone') if event_type == 'known' else ''
                                                except Exception:
                                                    name = 'Someone' if event_type == 'known' else ''
                                                if event_type == 'known':
                                                    filtered_lines.append(f"👤 {name} detected at {rel_time_f:.2f}s")
                                                elif event_type == 'unknown':
                                                    filtered_lines.append(f"🚨 Unknown detected at {rel_time_f:.2f}s")
                                                elif event_type == 'snap':
                                                    filtered_lines.append(f"📸 Snapshot at {rel_time_f:.2f}s")
                                    if filtered_lines:
                                        timeline = "\n".join(filtered_lines[-5:])
                                except Exception:
                                    timeline = ""
                        if timeline:
                            message += f"\n\nRecent Timeline (video-relative):\n{timeline}"

                        try:
                            with open(snap_filepath, 'rb') as f:
                                bot.send_photo(chat_id=TELEGRAM_CHAT_ID, photo=f, caption=message)
                        except Exception as e:
                            print(f"[ERROR] Failed to send known photo to Telegram: {e}")
                        # --- End photo send ---
                        # Removed duplicate text notification to avoid double message
...
def handle_telegram_commands():
    ...
    def set_servo_angle_cmd(update, context):
        try:
            ...
            if not (-90 <= angle <= 180):
                update.message.reply_text("❗ Angle must be between -90 and 180.")
                return
        except (IndexError, ValueError):
            update.message.reply_text("Usage: /servo_angle <-90~180>")
            return
        with app.test_client() as client:
...
/servo_angle <-90~180> - Move servo to specified angle
/snap - Take a snapshot and send to Telegram
/status - Show current status
/free_space - Show available disk space
/help - Show this help message
"""
        update.message.reply_text(help_msg)

    def free_space_cmd(update, context):
        try:
            total, used, free = shutil.disk_usage(VIDEO_DIR)
            free_gb = free / (1024 ** 3)
            total_gb = total / (1024 ** 3)
            percent = (free / total) * 100 if total > 0 else 0
            msg = f"💾 Free space: {free_gb:.2f} GB / {total_gb:.2f} GB ({percent:.1f}%)"
        except Exception as e:
            msg = f"❌ Failed to get free space: {e}"
    ...
    dp.add_handler(CommandHandler("free_space", free_space_cmd))
